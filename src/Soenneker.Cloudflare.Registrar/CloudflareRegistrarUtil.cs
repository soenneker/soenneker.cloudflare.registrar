...
public sealed class CloudflareRegistrarUtil : ICloudflareRegistrarUtil
{
    ...
    public async ValueTask<RegistrarApiDomainCheckResponse?> CheckDomain(string accountId, RegistrarApiDomainCheckRequest request,
    ...
    public async ValueTask<RegistrarApiWorkflowStatusResponseSingle?> CreateRegistration(string accountId, RegistrarApiRegistrationCreateRequest request,
        Action<RequestConfiguration<DefaultQueryParameters>>? requestConfiguration = null, CancellationToken cancellationToken = default)
    {
        RegistrarRequestBuilder registrar = await Get(accountId, cancellationToken).NoSync();
    ...
    public async ValueTask<RegistrarApiWorkflowStatusResponseSingle?> UpdateRegistration(string accountId, string domainName, RegistrarApiRegistrationUpdateRequest request,
        Action<RequestConfiguration<DefaultQueryParameters>>? requestConfiguration = null, CancellationToken cancellationToken = default)
    {
        RegistrarRequestBuilder registrar = await Get(accountId, cancellationToken).NoSync();

        return await registrar.Registrations[domainName].PatchAsync(request, requestConfiguration, cancellationToken).NoSync();
    }

    public async ValueTask<RegistrarApiWorkflowStatusResponseSingle?> GetRegistrationStatus(string accountId, string domainName,
        Action<RequestConfiguration<DefaultQueryParameters>>? requestConfiguration = null, CancellationToken cancellationToken = default)
    {
        RegistrarRequestBuilder registrar = await Get(accountId, cancellationToken).NoSync();

        return await registrar.Registrations[domainName].RegistrationStatus.GetAsync(requestConfiguration, cancellationToken).NoSync();
    }

    public async ValueTask<RegistrarApiWorkflowStatusResponseSingle?> GetUpdateStatus(string accountId, string domainName,
        Action<RequestConfiguration<DefaultQueryParameters>>? requestConfiguration = null, CancellationToken cancellationToken = default)
    {
        RegistrarRequestBuilder registrar = await Get(accountId, cancellationToken).NoSync();

        return await registrar.Registrations[domainName].UpdateStatus.GetAsync(requestConfiguration, cancellationToken).NoSync();
    }
}
